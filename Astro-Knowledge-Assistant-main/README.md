# 📡 SSGI Space Intelligence System
...
1. **Ingestion** → NASA DONKI API + NOAA text files
2. **Cleaning** → `clean_nasa.py`, `refine_noaa.py`
3. **Chunking** → Domain-specific `solar_chunker.py`
4. **Embedding & Indexing** → FAISS + Sentence Transformers
5. **Retrieval** → Smart filtering + Cross-Encoder reranking
6. **Generation** → Grounded Llama 3 briefing
